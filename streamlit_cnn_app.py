...
mnist_transform = transform.Compose(
    [transform.ToImage(), transform.ToDtype(torch.float32, scale=True)]
)
...
st.markdown("`X_test`의 실제 손글씨 샘플을 선택하여 PyTorch CNN 모델이 예측합니다.")
...
if model is not None:
    ...
    sample_index = st.slider(
        "샘플 인덱스 선택", min_value=0, max_value=len(test_dataset) - 1, value=0
    )

    image, label = test_dataset[sample_index]

    st.image(image.squeeze(0).numpy(), caption=f"실제 숫자: {label}", width=150)
    ...
    st.bar_chart(probability.cpu().numpy())

else:
    st.warning("PyTorch 모델(.pth)이 없습니다. 먼저 학습을 완료해주세요.")
