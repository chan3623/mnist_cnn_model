...
def preprocess_image(image_arr):
    ...
    shift_y = int(round(binary.shape[0] // 2 - cy))

    shift_x = int(round(binary.shape[1] // 2 - cx))

    shifted = shift(binary, shift=(shift_y, shift_x), mode="constant", cval=0)

    # 28x28
    resized = cv2.resize(
        shifted.astype("uint8"), (28, 28), interpolation=cv2.INTER_AREA
    )
...
st.markdown("흰 종이에 검은색 펜으로 0~9 숫자를 작성한 후 웹캠으로 촬영합니다.")
...
if model is None:
    ...
    if image_data is not None:
        ...
        st.image(image, caption="입력 이미지", width=500)
        ...
        processed = preprocess_image(gray_np)

        if processed is None:
            st.error("숫자 영역을 찾지 못했습니다.")
            ...
            # 전처리 이미지
            # ----------------------------------------------
            st.subheader("전처리된 MNIST 입력 이미지")
